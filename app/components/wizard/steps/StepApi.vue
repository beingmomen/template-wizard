...
<template>
  <div class="space-y-6">
        ...
        <div class="space-y-4 w-full">
          ...
          >
            <div class="flex items-center gap-2">
              <UIcon name="i-lucide-folder-open" class="w-5 h-5 text-primary-500" />
              <span class="font-medium text-gray-900 dark:text-white">{{ group.name || 'مجموعة جديدة' }}</span>
              <UBadge size="xs" variant="subtle">{{ group.endpoints?.length || 0 }} endpoints</UBadge>
            </div>
            <div class="flex items-center gap-2">
              <UIcon
          ...
          <!-- Collapsible Content -->
          <div v-show="isGroupOpen(groupIndex)" class="space-y-4 pt-2">
            <!-- Group Info -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="space-y-3">
            ...
            <!-- Endpoints List -->
            <div v-if="group.endpoints?.length > 0" class="space-y-2">
              <div
              ...
              >
                ...
                <!-- Permissions -->
                <UFormField v-if="endpoint.authRequired && hasPermissions" label="الصلاحيات المطلوبة">
                ...
                <div v-if="endpoint.method === 'GET'" class="space-y-3">
                  <div class="flex items-center justify-between">
                    <label class="text-sm font-medium text-gray-600 dark:text-gray-400">
                      Query Parameters
                    </label>
                  ...
                  <!-- Query Params List -->
                  <div v-if="endpoint.queryParameters?.length > 0" class="space-y-2">
                    <div
            ...
            >
              <UIcon name="i-lucide-network" class="w-8 h-8 mx-auto text-gray-400" />
              <p class="mt-2 text-sm text-gray-500">لا توجد endpoints في هذه المجموعة</p>
              <UButton
                size="xs"
                variant="soft"
                icon="i-lucide-layers"
    ...
    <div v-if="state.endpoints?.length > 0" class="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
    ...
    <UModal :open="groupDeleteConfirm.show" @update:open="groupDeleteConfirm.show = $event">
      <template #content>
        <div class="p-6">
          <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">تأكيد حذف المجموعة</h3>
          </div>
          ...
          <div class="flex justify-end gap-3 mt-6">
            <UButton variant="ghost" label="إلغاء" @click="groupDeleteConfirm.show = false" />
            <UButton color="error" icon="i-lucide-trash-2" label="حذف" @click="executeGroupDelete" />
          </div>
        </div>
      </template>
    </UModal>
